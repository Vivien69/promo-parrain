						...
						$.getJSON(options.src+"?q="+q, function(data, textStatus) {
							...
							if (textStatus == 'success')
							{
								...
								if (!has_data)
								{
									...
									output += '<p class="'+options.classPrefix+'header-limit">0 resultat</p>'+"\n";
									output += '</li>';
								}
								else
								{
									$.each(data, function(i, group) {
										...
										if (options.showEmptyCategories || (!options.showEmptyCategories && group['data'].length != 0))
										{
											...
											var fill_code = (options.fillBox) ? 'document.getElementById(\''+inputObj.attr('id')+'\').value = \'%\';document.getElementById(\'infodep\').value = \'$\';document.getElementById(\'typedep\').value = \'#\';' : '';
											$.each(group['data'], function (j, item) {
												if (count < limit)
												{
													...
													link_open += '" ';
													link_open += (item['onclick'] != undefined) ? ' onclick="'+fill_code.replace("%", item[options.fillBoxWith]).replace("$", item['secondary']).replace("#", item['type'])+(options.executeCode ? item['onclick'] : '')+'" ' : '';
													link_open += '>';
