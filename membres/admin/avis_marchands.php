...
while($row = $prep->fetch(PDO::FETCH_ASSOC)) {
	...
	$GLOBALS['nb_req']++;
	if($prepa->rowcount() == 1) {
		$sql = $pdo->prepare("DELETE FROM comments_marchands WHERE id = ".$id);
		if ($sql->execute()) {
			$GLOBALS['nb_req']++;
...
if (isset($_GET["verified"]) && $_GET["verified"] == 'ok') {
	...
	$GLOBALS['nb_req']++;
	if($prepa->rowcount() == 1) {
		$sql = $pdo->prepare("UPDATE comments_marchands SET verif = 1 WHERE id = ".$id);
		if ($sql->execute()) {
			$GLOBALS['nb_req']++;
	...
	$check_contents = $pdo->query('SELECT COUNT(*) as total FROM comments_marchands');
									$GLOBALS['nb_req']++;
	...
	{	
		...
		$pagination_system.= "</div>";			
			...
			$sql= 'SELECT C.id, C.id_sender, C.id_marchand, C.date, C.commentaire, C.verif, US.membre_id, US.membre_utilisateur AS mu, M.id as idmarchand, M.img, M.nom_marchand, M.cat FROM comments_marchands C
				LEFT JOIN user US ON C.id_sender = US.membre_id
                LEFT JOIN marchands M ON C.id_marchand = M.id
				  ORDER BY date DESC
  ...
  <div class="block_inside">
  	...
  	if ($prep->rowcount() >= 1) {
		...
		echo '<div style="height:30px;width:100%;background:#EEE;border:1px solid #CCC;display:flex;padding:5px 0">
				...
				<div style="width:15%;line-height:30px;text-align:center;border-right:1px solid #CCC;text-transform:uppercase;font-weight:bold;">Marchand</div>
	...
	echo '<div class="list-item" style="border-bottom:1px solid #CCC;display:block;text-align:left;position:relative;">
				...
				<!--<p class="titre_message_utilisateur" style="margin-bottom:0;><a href="'.ROOTPATH.'/membres/admin/messagesvoir.php?message='.$result['id'].'">Avis n°'.$result['id'].'</a></p>-->
				...
				<button type="button" style="margin-top:5px;" class="btn btn-info gerer">Gérer <i style="margin-left:10px" class="fa fa-angle-down fa-lg"></i></button>
					<div class="vpb_down_triangle" style="margin:34px 0px 0 0px;">
						<div class="vpb_down_triangle_inner">
							<div><a href="'.ROOTPATH.'/membres/admin/avis-marchands/'.$current_page.'/'.$result['id'].'-no/ok" onclick="if(!confirm(\'Etes-vous sur de vouloir traiter ce message ?\')) return false;"><i style="margin-right:10px;" class="fa fa-check fa-lg"></i>Confirmer</a></div>
						</div>
						<div class="vpb_down_triangle_inner">
							<div><a href="'.ROOTPATH.'/membres/admin/avis_edit.php?id='.$result['id'].'&type=1"><i style="margin-right:10px;" class="fa fa-pen fa-lg"></i>Modifier l\'avis</a></div>
						</div>
						<div class="vpb_down_triangle_inner">
							<div><a href="'.ROOTPATH.'/membres/admin/avis-marchands/'.$current_page.'/'.$result['id'].'-ok" onclick="if(!confirm(\'Etes-vous sur de vouloir supprimer ce message ?\')) return false;"><i style="margin-right:10px;" class="fa fa-times fa-lg"></i> Supprimer</a></div>
						</div>
					</div>
				<div style="display:flex;justify-content:space-between">
					...
					<div style="width:15%;text-align:center;border-right:1px solid #CCC;"><a href="'.ROOTPATH.'/'.format_url(find_categorie($result['cat'])).'-'.$result['cat'].'/'.format_url($result['nom_marchand']).'-'.$result['idmarchand'].'/parrainage"><div class="item-img" style="margin:0;background-image: url(\'' . ROOTPATH . '/membres/includes/uploads-img/120-' . $result['img'] . '\');"></div></a></div>
					<div style="line-height:40px;width:15%;height:40px;text-align:center;border-right:1px solid #CCC;font-weight:bold;"><a href="'.ROOTPATH.'/profil/'.$result['membre_id'].'#comment">'.$result['mu'].'</a></div>
