...
if ($prep->rowCount() == 0) {
	...
	while ($row = $prep->fetch(PDO::FETCH_ASSOC)) {
        ...
        if (isset($_GET["del"]) && $_GET['del'] && isset($_GET["id"]) && is_numeric($_GET["id"])) {
...
// PAGINATION
$check_contents = $pdo->query('SELECT COUNT(*) as total FROM execparrainages WHERE (id_parrain = '.$_SESSION['membre_id'].' AND deleted_parrain = 3) OR (id_filleul = '.$_SESSION['membre_id'].' AND deleted_filleul = 3)');
$GLOBALS['nb_req']++;
...
WHERE (EP.id_filleul = '.$_SESSION['membre_id'].' AND deleted_filleul = 3) OR (EP.id_parrain = '.$_SESSION['membre_id'].' AND deleted_parrain = 3)
ORDER BY date DESC
...
?>
	...
	<span></span>
        ...
        <div class="action">
            <a href="<?=ROOTPATH.'/membres/parrainages_archives?del=1&id='.$row['idnotif'] ?>" onclick="if(!confirm(\'Etes vous sur de supprimer ce parrainage ?\')) return false;" title="Supprimer le parrainage" class="button">Supprimer</a>
        </div>
